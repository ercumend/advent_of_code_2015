...
WHILE @xidx < 1000 BEGIN 
    WHILE @yidx < 1000 BEGIN 
        INSERT INTO @lightList (x,y,isLit)
        SELECT @xidx,@yidx,0   
        SET @yidx = @yidx + 1
    END
    SET @yidx = 0
...
WHILE CHARINDEX(CHAR(13),@input,0) > 0 BEGIN
    ...
    UPDATE LL
    SET LL.isLit = CASE WHEN @Command = 'turn on' THEN 1 
                        WHEN @Command = 'turn off' THEN 0
                        WHEN @Command = 'toggle' THEN ~LL.isLit
                        ELSE NULL
                   END
    FROM @lightList LL
...
SELECT COUNT(*) FROM @lightList LL
WHERE LL.isLit = 1
