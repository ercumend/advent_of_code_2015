SET NOCOUNT ON
DECLARE @input varchar(max) = N'turn on 489,959 through 759,964
...
WHILE @xidx < 1000 BEGIN 
    WHILE @yidx < 1000 BEGIN 
        INSERT INTO @lightList (x,y,brightness)
        SELECT @xidx,@yidx,0   
        SET @yidx = @yidx + 1
    END
    SET @yidx = 0
...
WHILE CHARINDEX(CHAR(13),@input,0) > 0 BEGIN
    ...
    SELECT @Command = commandlist.command FROM (VALUES ('turn on',1),('turn off',2),('toggle',3)) commandlist(command,id) WHERE PATINDEX('%' + commandlist.command + '%', @expression) > 0

    SET @expression = STUFF(@expression,1,LEN(@Command) + 1,'')
    SET @expression = REPLACE(@expression,'through','|')

    SET @x1 = LEFT(@expression,CHARINDEX(',',@expression,0) -1 )
    SET @expression = STUFF (@expression,1,CHARINDEX(',',@expression),'')
    SET @y1 = LEFT(@expression,CHARINDEX('|',@expression,0) -1 )
    SET @expression = STUFF (@expression,1,CHARINDEX('|',@expression),'')
    SET @x2 = LEFT(@expression,CHARINDEX(',',@expression,0) -1 )
    SET @expression = STUFF (@expression,1,CHARINDEX(',',@expression),'')
    SET @y2 = @expression


    UPDATE LL
    SET LL.brightness = CASE WHEN @Command = 'turn on' THEN LL.brightness  + 1  
                             WHEN @Command = 'turn off' AND LL.brightness > 0 THEN LL.brightness  - 1  
                             WHEN @Command = 'turn off' AND LL.brightness = 0 THEN 0
                             WHEN @Command = 'toggle' THEN LL.brightness  + 2  
                             ELSE NULL
                   END  
    FROM @lightList LL
    WHERE LL.x BETWEEN @x1 AND @x2
      AND LL.y BETWEEN @y1 AND @y2
END 

SELECT LL.* FROM @lightList LL
WHERE LL.brightness < 0


SELECT SUM(LL.brightness )
FROM @lightList LL
WHERE LL.brightness > 0
