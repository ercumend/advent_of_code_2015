SET NOCOUNT ON
DECLARE @input varchar(max) = N'Sue 1: cars: 9, akitas: 3, goldfish: 0
...
DECLARE @SueList TABLE (Name varchar(50) , children int, cats int, samoyeds int, pomeranians int, akitas int, vizslas int, goldfish int, trees int, cars int, perfumes int, isPossible bit)

DECLARE @Name varchar(50) , @children int, @cats int, @samoyeds int, @pomeranians int, @akitas int, @vizslas int, @goldfish int, @trees int, @cars int, @perfumes int


WHILE PATINDEX('%' + char(10) + '%',@input) > 0 BEGIN 
    SELECT @Expression = LEFT(@input,PATINDEX('%' + char(10) + '%',@input)-2)
    
    SET @Name = LEFT(@Expression,CHARINDEX(':',@Expression) - 1)
    SET @Expression = STUFF(@Expression,1,CHARINDEX(':',@Expression),'')

    PRINT @Expression 

    SET @children = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'children')
    SET @cats = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'cats')
    SET @samoyeds = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'samoyeds')
    SET @pomeranians = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'pomeranians')
    SET @akitas = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'akitas')
    SET @vizslas = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'vizslas')
    SET @goldfish = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'goldfish')
    SET @trees = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'trees')

    SET @cars = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'cars')
    SET @perfumes = (SELECT RIGHT(S.Item, LEN(S.Item) - CHARINDEX(':',S.Item)  )  FROM fn_Split(@Expression,',') S WHERE LEFT(S.Item,CHARINDEX(':',S.Item) -1 ) = 'perfumes')


    INSERT INTO @SueList (Name,children, cats, samoyeds, pomeranians, akitas, vizslas, goldfish, trees, cars, perfumes)
    SELECT @Name, @children, @cats, @samoyeds, @pomeranians, @akitas, @vizslas, @goldfish, @trees, @cars, @perfumes
    
    SET @input = STUFF(@input,1,PATINDEX('%' + char(10) + '%',@input),'')
END


UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.children IS NOT NULL AND SL.children <> 3

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.cats IS NOT NULL AND SL.cats <> 7

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.samoyeds IS NOT NULL AND SL.samoyeds <> 2

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.pomeranians IS NOT NULL AND SL.pomeranians <> 3

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.akitas IS NOT NULL AND SL.akitas <> 0

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.vizslas IS NOT NULL AND SL.vizslas <> 0

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.goldfish IS NOT NULL AND SL.goldfish <> 5

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.trees IS NOT NULL AND SL.trees <> 3

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.cars IS NOT NULL AND SL.cars <> 2

UPDATE SL SET SL.isPossible = 0 FROM @SueList SL WHERE SL.perfumes IS NOT NULL AND SL.perfumes <> 1


SELECT * 
FROM @SueList SL
WHERE ISNULL(SL.isPossible,1) <> 0
